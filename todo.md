# Project: Next.js + Supabase Starter App Checklist

## Todo List

### 1. Project Initialization

- [x] Initialize Next.js app (latest version) with TypeScript and Tailwind CSS.
- [x] Organize folder structure (`/components`, `/lib`, `/app`, `/hooks`, `/types`).
- [x] Initialize Supabase: `npx supabase init`.
- [x] Install dependencies: `@supabase/ssr` `@supabase/supabase-js`.

### 2. Database & Security (The "Source of Truth")

- [ ] **Spin up local engine**: `npx supabase start` (Needed to generate diffs).
- [ ] Create `supabase/schemas/profiles.sql`:
  - [ ] Define `profiles` table (PK references `auth.users`).
  - [ ] Add `updated_at` and `email` fields.
- [ ] Create PostgreSQL Trigger:
  - [ ] Function to handle `AFTER INSERT` on `auth.users`.
  - [ ] Trigger to sync email/ID to `profiles`.
- [ ] Configure RLS Policies:
  - [ ] Enable RLS on `profiles`.
  - [ ] Policies for SELECT, UPDATE, and INSERT (own profile only).
- [ ] **Generate Migration**:
  - [ ] Run `npx supabase db diff --local > supabase/migrations/00_initial_schema.sql`.
  - [ ] Verify migration file contains the table, trigger, and RLS.

### 3. Supabase Integration (SSR)

- [ ] Create `lib/supabase/`: `client.ts`, `server.ts`, and `middleware.ts`.
- [ ] Implement `middleware.ts` in root for session handling.
- [ ] Create reusable Auth patterns (`useAuth` hook & server helpers).

### 4. UI & Routes

- [ ] **Home Page (`/`)**: Welcome + Auth status.
- [ ] **Auth Pages**: `/login` and `/signup` with error handling.
- [ ] **Dashboard (`/dashboard`)**: Protected route + Sign out.
- [ ] **Profile Page (`/profile`)**:
  - [ ] Form for metadata updates.
  - [ ] **Avatar Upload**: Storage bucket setup + upload logic + `avatar_url` DB update.
...
- [ ] **Setup Script (`setup.sh` or `setup.js`)**:
  - [ ] `npm install`.
  - [ ] `npx supabase start`.
  - [ ] Automate credential extraction to `.env.local`.
  - [ ] `npx supabase db reset` (to test migrations).
  - [ ] Ensure idempotency.
- [ ] **GitHub Actions**:
  - [ ] Migration workflow for production push.

### 6. Final Documentation & Cleanup

- [ ] Write `README.md` (Quick start, structure, Auth flow, Deployment).
- [ ] **Clean Slate Test**: Delete `node_modules` and `.env.local`, run `setup.sh`.
- [ ] **Final Step**: Remove `node_modules` before submission.

## Original Assignment Description

Assignment 2: Next.js + Supabase Starter App
...
Test the entire setup process from scratch (delete node_modules and .env.local, then run your setup script—do not delete the supabase directory as it contains your migrations and schemas)
Before you submit, remove the node_modules folder
Submit your project
